...
def cn_stock_strategy_factor_process(sql_factor):
    sql = f"""
    WITH
    data_alpha AS (
        {sql_factor}
...
def cn_stock_strategy_multi_factor_synthesize(factos_dict):
    sql_list1 = []
    sql_list2 = []
    sql_list3 = []
...
def cn_stock_strategy_factor_trade_sql(
    sql_factor, 
    number_security=10, 
    ...
    data_date AS (
        ...
        FROM data_filter JOIN cn_stock_valuation USING (date, instrument) JOIN mldt_cn_trading_calendar USING (date)
        WHERE {str_rebalance} 
    )
    SELECT *
    FROM data_date
...
def cn_stock_strategy_factor_backtest(df, capital_base):
