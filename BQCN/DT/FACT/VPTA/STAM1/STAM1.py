...
class CNDT_FACT_VPTA_STAM1(VPTA):
    
    def __init__(self, sd, ed, instruments = []):
        super().__init__(sd, ed, instruments)

    def get_alpha_sql(self, sql_alpha):
        ins_str = f"WHERE instrument IN {str(tuple(self.instruments))}" if self.instruments != [] else ""
        sql_combine = f"""
        WITH 
        data_base AS (
            SELECT
    ...
    def get_alpha_df(self, sql_alpha, lag):
        return self.get_dai_df(self.get_alpha_sql(sql_alpha), self.sd, self.ed, lag_min = lag)
